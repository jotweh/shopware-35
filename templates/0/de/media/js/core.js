...
function updateLiveshopping(uniquekey, ordernumber, max_quantity_enable, max_quantity, sells, minPrice, typeID, valid_to_ts)
{	
	var now = sServer.dateObj;
	var target 	= new Date();
	target.setTime(valid_to_ts * 1000);
	
	var d = uniquekey+ordernumber+'_days';
	var h = uniquekey+ordernumber+'_hours';
	var m = uniquekey+ordernumber+'_min';
	var s = uniquekey+ordernumber+'_sec';
	var dp = uniquekey+ordernumber+'_days_prozess';
	var hp = uniquekey+ordernumber+'_hours_prozess';
	var mp = uniquekey+ordernumber+'_min_prozess';
	var sp = uniquekey+ordernumber+'_sec_prozess';
	var spbar = uniquekey+ordernumber+'_secbar_prozess';
	var instockp = uniquekey+ordernumber+'_instock_prozess';
	...
	{		
		...
		try{	
			if (diff.s == 0) {
				if(typeID == 2 || typeID == 3) {
					...
					$(document.body).getElements('span[class='+uniquekey+ordernumber+'_price'+']').each(function(item, index, allItems){
						newPrice = item.innerHTML;
					});
					$(document.body).getElements('span[class='+uniquekey+ordernumber+'_display_price'+']').each(function(item, index, allItems){
						newPrice2 = newPrice;
						if(typeID == 3) {
						newPrice2 = eval(newPrice2)+eval(minPrice);
						}else{
							newPrice2 = eval(newPrice2)-eval(minPrice);
						}
						item.innerHTML = number_format(newPrice2, 2, ',','.');
					});
					$(document.body).getElements('span[class='+uniquekey+ordernumber+'_price'+']').each(function(item, index, allItems){
						$(document.body).getElements('span[class='+uniquekey+ordernumber+'_lastupdatemin'+']').each(function(item, index, allItems){
							var lastupdate = item.innerHTML;
						});
						item.innerHTML = newPrice2;
					});
				}
			}
		}catch(e){}
		...
		if (max_quantity_enable) {
			$(document.body).getElements('div[class='+instockp+']').each(function(item, index, allItems){
				//var proz = eval(max_quantity)*100/eval(max_quantity)+eval(sells);
				var proz = eval(max_quantity)*100;
				var total = eval(max_quantity)+eval(sells);
				proz = proz/total;
				proz = 100 - proz;
				item.setStyle('width', proz+'%');
			});
		...
		try{
			var crnt_ordernumber = $('selected_ordernumber').innerHTML;
			if(crnt_ordernumber != '')
			{
				$('article_details').setHTML($(crnt_ordernumber).innerHTML);
			}		
		}catch(e){}
	}
...
var Search = {
	init: function(){
		...
		this.fx2 = new Fx.Styles('searchresults', {duration:0, wait:false});
		//$('searchfield').focus();
		$('searchfield').autocomplete = 'off';
		$('searchfield').addEvent('keyup', function(event)
		{
			Search.timer = $clear(Search.timer);
			Search.timer = Search.doSearch.delay(300);
		});
	...
	doSearch: function(){
		...
		if(Search.ajax)
			Search.ajax.cancel();
		if(!$('searchfield').getValue()||$('searchfield').getValue()==''||$('searchfield').getValue().length<minsearchlenght)
			return;
		this.ajax = new Ajax(Search.url, {
			method: 'post',
			onComplete: function(request)
			{
				if(request) var response = Json.evaluate(request);
				if(response.sResults&&response.sResults!=null&&response.sResults!='')
				{
					var result = response.sResults;
					$('searchresults').setOpacity(0).setStyle('display','block');
		...
		}).request($("searchform").toQueryString());
	},
	...
	buildSearchResults: function(results){
		...
		var link = new Element('a', {'class': 'resultall', 'style': 'z-index:100;'}).setHTML(sSearchShowAllResults).injectInside(div);
		link.addEvent('click', function(e) {
			$('searchform').submit();
		});

		//		var heading = new Element('h2').setHTML('Suchergebnisse:').injectInside(div);
		var list = new Element('ul').injectInside(div);
		div.injectInside(container.empty());		
		...
		{
			var li = new Element('li', {'style': '', 'class': 'searchresult_supplier'}).injectInside(list);
			var counter = 0;
			...
			results.each(function(result, i) {
				...
				{	
					...
					if(counter==0)
					{
						...
						var heading = new Element('h2').setHTML(sSearchManufacturer).injectInside(div);
						div.injectInside(li);
					}
								
					if(result.image)
					{
						var imagelink = new Element('a', {'href': result.link, 'class': 'searchthumb', 'title': result.name}).injectInside(li);
						var image = new Element('img', {'src': result.image, /*'alt': result.name,*/ 'class': 'resultimage', 'style': 'margin: 0pt 5px 0pt 0pt'}).injectInside(imagelink);
					}else{
						var titlelink = new Element('a', {'href':result.link, 'class': 'resultlink'}).injectInside(li);
						var title = new Element('h3').setHTML(result.name).injectInside(titlelink);
					}
					counter++;
				}	
			});
		...
		{
			var li = new Element('li', {'style': '', 'class': 'searchresult_cat'}).injectInside(list);
			var list2 = new Element('ul').injectInside(li);
			var counter = 0;		
			results.each(function(result, i) {
				if(result.type == "category")
				{
					if(counter==0)
					{
						var li2 = new Element('li', {'style': '', 'class': 'header'}).injectInside(list2);
						
						var div = new Element('div', {'style': 'clear:both;'});
						var heading = new Element('h2').setHTML(sSearchCategories).injectInside(div);
						div.injectInside(li2);
						
						var li2 = new Element('li', {'style': '', 'class': 'searchresult'}).injectInside(list2);
					}else{
						var li2 = new Element('li', {'style': '', 'class': 'searchresult'}).injectInside(list2);
					}
					...
					if(result.image)
					{
						var imagelink = new Element('a', {'href': result.link, 'class': 'searchthumb', 'title': result.name}).injectInside(li2);
						var image = new Element('img', {'src': result.image, /*'alt': result.name,*/ 'class': 'resultimage', 'style': 'margin: 0pt 5px 0pt 0pt'}).injectInside(imagelink);
					}
					
					var titlelink = new Element('a', {'href':result.link, 'class': 'resultlink'}).injectInside(li2);
					var title = new Element('h3').setHTML(result.name).injectInside(titlelink);
					
					var titlesize = title.getSize();
					if(titlesize.scrollSize.x>titlesize.size.x)
					{
	//					var titlerelated = new Element('div',{'class': 'related'}).setHTML("...").injectInside(title);
					}
					if(result.description)
					{
						var description = new Element('div',{'class': 'searchdescription'}).setHTML(result.description).injectInside(li2);
						var descriptionsize = description.getSize();
						if(descriptionsize.scrollSize.y>descriptionsize.size.y)
						{
	//						var descriptionrelated = new Element('div',{'class': 'related'}).setHTML("...").injectInside(description);
						}
					}
					counter++;
				}
		...
		{
			var li = new Element('li', {'style': '', 'class': 'searchresult'}).injectInside(list);
			var list2 = new Element('ul').injectInside(li);
			var counter = 0;		
			results.each(function(result, i) {
...
var sServer = function(){
	...
	return {
		init : function(){
			sServer.dateObj = new Date();
			sServer.dateObj.setTime(sServerTime * 1000);
		},
		increment : function(){
			//Gespeicherte Zeit + 1 Sekunde
			var crntTime = sServer.dateObj.getTime()+1000;
			sServer.dateObj = new Date(crntTime);
		}
	}
}();
sServer.init();
