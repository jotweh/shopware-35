...
<div class="listing_box">
    ...
    <div class="listing_box_top">
        ...
        <div class="article-options clearfix">
            ...
            {/if}
			{if !$sOffers OR $_GET.sPage}
			{* PAGE_SORT *}
			<form name="frmsort" method="POST" action="" id="frmsort" style="z-index:-1;">
				<input type="hidden" name="cID" value="{$cID}" />
				<input type="hidden" name="cMan" value="{$cMan}" />
				<label for="cSORT" style="width: 60px; height: 20px;">{* sSnippet: sort *}{$sConfig.sSnippets.sArticlesort}</label>
				<select name="sSort" id="cSORT" onChange="$('frmsort').submit()" style="z-index:-1;">
...
<div style="padding: 0px;">
	...
	{$sSupplierInfo.name}
	{/if} 
	<a href="{$sSupplierInfo.link}" title="{* sSnippet: show all manufacturers *}{$sConfig.sSnippets.sArticleshowallmanufacturers}" class="bt_allsupplier">{* sSnippet: show all manufacturers *}{$sConfig.sSnippets.sArticleshowallmanufacturers}</a>
	</div>
...
{if $sOffers AND !$_GET.sPage}
	{foreach from=$sOffers item=offer key=key name="counter"}
			...
			{else}
				{include file="articles/article_box_4col.tpl" sArticle=$offer}
			{/if}	
	{/foreach}
{else}
	{foreach from=$sArticles item=article key=key name="counter"}
		{include file="articles/article_box_4col.tpl" sArticle=$article key=$key}
	{/foreach}
{/if}
