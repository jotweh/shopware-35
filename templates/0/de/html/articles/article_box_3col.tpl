{* ARTBOX *}
<div class="artbox {cycle values="listing_grid_left,listing_grid_middle,listing_grid_right"}">

{if $sArticle.newArticle}<!-- ico new --><div class="ico_new"><span class="hidden">{* sSnippet: new *}{$sConfig.sSnippets.sArticlenew}</span></div><!-- /ico new -->{/if}
{if $sArticle.highlight}<!-- ico tipp --><div class="ico_tipp"><span class="hidden">{* sSnippet: tip *}{$sConfig.sSnippets.sArticletip}</span></div><!-- /ico tipp -->{/if}
{if $sArticle.esd}<div class="ico_esd"><span class="hidden">{* sSnippet: Available as an immediate download *}{$sConfig.sSnippets.sArticletipavailableasanimmedi}</span></div>{/if}
	...
	{* ARTICLE PICTURE *}
	<div class="top">
	<a href="{$sArticle.linkDetails}" title="{$sArticle.articleName|replace:"&":"&"}" class="artbox_thumb" {if $sArticle.image.src}style="background:url({$sArticle.image.src.3}) no-repeat center center;"{/if}>{if !$sArticle.image.src}<img src="../../media/img/de/layout/no_picture.jpg" alt="{* sSnippet: No picture available *}{$sConfig.sSnippets.sArticlenoPicture}" />{/if}</a>
	{* /ARTICLE PICTURE *}
	
	{* ARTICLE COMPARE / MORE *}
	    	{if !$sHideCompare}<a href="#" onclick="addCompare('{$sArticle.articleID}')" class="compare_artbox3">{$sConfig.sSnippets.sArticleButtonCompare}</a>{/if}
	<a href="{$sArticle.linkDetails}" title="{$sArticle.articleName|wordwrap:29:"":true}" class="more_artbox3">{$sConfig.sSnippets.sArticleButtonMore}</a>
	{* /ARTICLE COMPARE / MORE *}
