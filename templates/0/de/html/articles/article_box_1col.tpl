...
{if $sArticle.esd}<div class="ico_esd"><span class="hidden">{* sSnippet: Available as an immediate download *}{$sConfig.sSnippets.sArticleavailableasan}</span></div>{/if}
...
<div class="left">
    ...
    {* ARTICLE PICTURE *}
	<a href="{$sArticle.linkDetails}" title="{$sArticle.articleName|replace:"&":"&"}" class="artbox_thumb" {if $sArticle.image.src} style="background: url({$sArticle.image.src.2}) no-repeat center 20px;"{/if}>
    {if !$sArticle.image.src}<img src="../../media/img/de/layout/no_picture.jpg" alt="{* sSnippet: No picture available *}{$sConfig.sSnippets.sArticlenoPicture}" />{/if}
    </a>
	{* /ARTICLE PICTURE *}
...
{* ARTICLE COMPARE / MORE *}
	{if !$sHideCompare}<a href="#" onclick="addCompare('{$sArticle.articleID}')" class="compare_artbox1">{$sConfig.sSnippets.sArticleButtonCompare}</a>{/if}
	<a href="{$sArticle.linkDetails}" title="{$sArticle.articleName|wordwrap:29:"":true}" class="more_artbox1">{$sConfig.sSnippets.sArticleButtonMore}</a>
{* /ARTICLE COMPARE / MORE *}
	{* ARTICLE NAME *}
	<a href="{$sArticle.linkDetails}" title="{$sArticle.articleName|wordwrap:29:"":true}" class="headline">{$sArticle.articleName|truncate:49}</a>
	{* /ARTICLE NAME *}
    
	{* ARTICLE DESCRIPTION *}
	<div class="article-description">
		<p>{$sArticle.description_long|replace:"<b>":""|replace:"<strong>":""|replace:"</strong>":""|replace:"</b>":""|replace:"<B>":""|replace:"<STRONG>":""|replace:"</STRONG>":""|replace:"</B>":""|replace:"&":"&"|truncate:120}</p>	
