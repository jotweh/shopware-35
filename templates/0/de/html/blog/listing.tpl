...
<div>
	...
	<img src="{$sSupplierInfo.image}" alt="{$sSupplierInfo.name}" name="{$sSupplierInfo.name}" border="0" id="aktionsname" title="{$sSupplierInfo.name}" /><br />
	{else}
	{$sSupplierInfo.name}<br />
	{/if} 
	<a href="{$sSupplierInfo.link}" title="{$sCategoryContent.description}" class="bt_allsupplier">{* sSnippet: overview *}{$sConfig.sSnippets.sBlogShowAllAuthors}</a>
	</div>
...
{if $sOffers AND !$_GET.sPage}
	{foreach from=$sOffers item=offer key=key name="counter"}
			{if $offer.mode == "gfx"}
				{include file="blog/image.tpl" sArticle=$offer}
			{else}
				{include file="blog/col.tpl" sArticle=$offer}
			{/if}	
	{/foreach}
{else}
	{foreach from=$sArticles item=article key=key name="counter"}
		{include file="blog/col.tpl" sArticle=$article key=$key}
	{/foreach}
{/if}
