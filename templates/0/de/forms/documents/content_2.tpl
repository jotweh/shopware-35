      ...
      <table cellpadding="0" width="100%">
       ...
       {/if}
        <tr>
          ...
          {else}
          <td valign="top" align="right">{$Entry.netto*-1|string_format:"%.2f"|replace:".":","} {$sCurrency}</td>
          {/if}
          {/if}
          {if $sBillingData.netto != true&&$sBillingData.netto2 != true}
	          {if $Entry.price>0}
	          ...
	          {else}
	          <td valign="top" align="right">{$Entry.price*-1|string_format:"%.2f"|replace:".":","} {$sCurrency}</td>
	          {/if}
          {/if}
          {if $sBillingData.netto != true&&$sBillingData.netto2 != true}
	          {if $Entry.amount>0}
	          ...
	          {else}
	          <td valign="top" align="right">{$Entry.amount*-1|string_format:"%.2f"|replace:".":","} {$sCurrency}</td>
	          {/if}
	      {/if}
          {if $sBillingData.netto == true||$sBillingData.netto2 == true}
	          {if $Entry.amount_netto>0}
	           <td valign="top" align="right">-{$Entry.amount_netto|replace:".":","} {$sCurrency}</td>
	          {else}
	          <td valign="top" align="right">{$Entry.amount_netto*-1|string_format:"%.2f"|replace:".":","} {$sCurrency}</td>
	          {/if}
          {/if}
        </tr>
        {/foreach}
