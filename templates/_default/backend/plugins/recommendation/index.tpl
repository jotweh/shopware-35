...
Shopware.Recommendation.DetailForm = Ext.extend(Ext.FormPanel,
{
	labelWidth: 75, // label settings here cascade unless overridden
    frame:true,
    bodyStyle:'padding:5px 5px 0',
	...
	initComponent: function() {
		this.items = [
				new Ext.Panel(
				{
					...
					html: '<strong>Die Namen der gültigen Blöcke für die Positionierung auf der Detailseite, können Sie im Detailseiten Template einsehen (frontend/detail/index.tpl)</strong>',
					border: true,
					frame: true
				}
				),
	        	...
	        	{
	                fieldLabel: 'Block-Element zur Positionierung der Slider',
	                name: 'block_detail',
	                value: '{$block_detail}',
	                allowBlank:false,
	                labelStyle: 'width:287px;padding:0px 0px 0px 0px',
	            ...
	            , new Ext.form.Checkbox({
	            	...
	            	name: 'seen_active',
	            	checked: {$similary_viewed},
	            	labelStyle: 'width:287px;padding:0px 0px 0px 0px',
	            	fieldLabel: '"Kunden haben sich ebenfalls angeschaut" anzeigen',
	            	inputValue: 1
	            })
        	];
	        ...
	        this.buttons = [{
		            ...
		            handler: function(){
		            	...
		            	this.getForm().submit({ url: '{url module=backend controller=RecommendationAdmin action=saveDetail}'});
		            	
		            },
		            scope:this
	        }];
			Shopware.Recommendation.DetailForm.superclass.initComponent.call(this);
			this.url = '{url module=backend controller=RecommendationAdmin action=saveDetail}';
	}
}
);
Ext.QuickTips.init();
(function(){
	View = Ext.extend(Ext.Viewport, {
		...
		initComponent: function() {
		    ...
		    this.detailForm = new Shopware.Recommendation.DetailForm();
			...
			this.tabs = new Ext.TabPanel({
		        ...
		        items:[{
		                title: 'Detailseiten Konfiguration',
