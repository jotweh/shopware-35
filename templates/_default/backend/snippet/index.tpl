	...
	View = Ext.extend(Ext.Viewport, {
	    ...
	    deleteSnippets: function() {
	    	Ext.MessageBox.confirm('Bestätigung', 'Sollen die markierten Textbausteine wirklich gelöscht werden?', function deleteClientConfirmed(btn){
	    		if (btn=="yes") {
	    			...
	    			if(ids != "") {
	    				Ext.Ajax.request({
						   url: '{url module=backend controller=snippet action=deleteSnippets}',
						   success: function() {
	    ...
	    changeSnippets: function() {
	    	Ext.MessageBox.prompt('Namespace', 'Bitte tragen Sie den neuen Namespace ein:', 
          		function(btn,namespace) {
	      			...
	      			if(namespace != "" && ids != "") {
	          			Ext.Ajax.request({
						   url: '{url module=backend controller=snippet action=changeSnippets}',
						   success: function() {
						  		Ext.getCmp('snippet_grid').store.load();
						   },
	    ...
	    dublicateSnippets: function() {
	    	Ext.MessageBox.prompt('Namespace', 'Bitte tragen Sie einen neuen Namespace ein:', 
          		function(btn,namespace) {
          			...
          			if(namespace != "" && ids != "") {
	          			Ext.Ajax.request({
						   url: '{url module=backend controller=snippet action=dublicateSnippets}',
						   success: function() {
						  		Ext.getCmp('snippet_grid').store.load();
						   },
						   params: { 'snippetIds': ids,'nameSpace':namespace}
	    ...
	    updateDB: function(oGrid_Event,editData) {
            ...
            Ext.Ajax.request({   
                    ...
                    url: '{url module=backend controller=snippet action=changeSnippet}',
                    params: { 
                    	id: editData.id,
		...
		loadForm: function(grid,rowIndex,colIndex,e) {
			if(colIndex == 7) {
				...
				this.editForm.getForm().load({
					url:'{url module=backend controller=snippet action=loadSnippetForm}', 
					waitMsg:'Laden...',
	    ...
	    initComponent: function() {
	    	this.store = new Ext.data.Store({
    		   url: '{url module=backend controller=snippet action=getSnippet}',
            ...
            this.exportForm = new Ext.FormPanel({
		        ...
		        buttons: [{
		            ...
		            handler: function(){
			            ...
			            form.submit( { url:' {url module=backend controller=snippet action=exportSnippet}', success: function (el, r){
			            }});
			        }
		        }]
		    });
	    	...
	    	this.tree = new Ext.tree.TreePanel({
                ...
                loader: new Ext.tree.TreeLoader( { dataUrl:'{url module=backend controller=snippet action=getNS}' } )
			});
	    	...
	    	this.grid =  new Ext.grid.EditorGridPanel({
                ...
                columns: [
                        ...
                        editor: {
			            	...
			            	triggerAction: 'all',
			            	 store:  new Ext.data.Store({
								url: '{url module=backend controller=snippet action=getLocales}',
								autoLoad: true,
							   	reader: new Ext.data.JsonReader({
							    	root: 'locales',
							        id: 'id',
                    ...
                    },
                    {
                        ...
                        editor: {
			            	...
			            	triggerAction: 'all',
			            	 store:  new Ext.data.Store({
								url: '{url module=backend controller=snippet action=getshopIDs}',
								autoLoad: true,
							   	reader: new Ext.data.JsonReader({
							    	root: 'shopIDs',
							        id: 'shopIDs',
                ...
                bbar: {
		            ...
		            { 
		            	...
		            	triggerAction: 'all',
		            	 store:  new Ext.data.Store({
							url: '{url module=backend controller=snippet action=getLanguageShop}',
							autoLoad: true,
						   	reader: new Ext.data.JsonReader({
						    	root: 'locales',
						        id: 'id',
            ...
            this.editForm = new Ext.FormPanel({
            	...
            	},
            	{
	                ...
	                handler: function() {
						...
						if(form.isValid()){
							form.submit({ url: '{url module=backend controller=snippet action=submitSnippet}', waitMsg:'Speichern...',
