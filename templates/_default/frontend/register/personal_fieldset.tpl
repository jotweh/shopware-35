<div class="personal_settings">
	...
	{if !$update}
		{block name='frontend_register_personal_fieldset_skip_login'}
			{if !$sEsd && !$form_data.sValidation && !$this->config('NoAccountDisable')}
		        <div class="check">
	...
	{if !$update}
		...
		{block name='frontend_register_personal_fieldset_password_description'}
			<div class="fade_password description">
				{se name='RegisterInfoPassword'}{/se}{$this->config('MinPassword')} {se name='RegisterInfoPasswordCharacters'}{/se}<br /> {se name='RegisterInfoPassword2'}{/se}
			</div>
		{/block}
	{/if}
