...
{if $sArticle.images}
	...
	<div class="thumb_box">
		{if $sArticle.image.src.4}
			<a id="thumb{$sArticle.ordernumber}" 
			   href="{$sArticle.image.src.5}" 
			   title="{if $sArticle.image.description}{$sArticle.image.description}{else}{$sArticle.articleName}{/if}" 
			   ...
			   {if $this->config('sUSEZOOMPLUS')}class="cloud-zoom-gallery"{/if}
