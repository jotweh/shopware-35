...
{	/**
	...
	{		
		...
		// Now loop through basket, build table, make inserts and be happy ;9
		
		$sTable = new text_table();
		
		$sTableConfig = explode(";",$this->sSYSTEM->sCONFIG['sORDERTABLE']);
	
		foreach ($sTableConfig as $sTableConfigRow){
			$sTableConfigRow = explode(":",$sTableConfigRow);
			$sTable->add_col($sTableConfigRow[1],$sTableConfigRow[2],$sTableConfigRow[3]);
		}
		...
		foreach ($this->sBasketData["content"] as $basketRow){
			...
			// Needed for 2.0.4 compatibility purposes
			$sTable->add_row(array(
			"0"=>$position,
			"1"=>$basketRow["ordernumber"],
			"2"=>$basketRow["articlename"],
			"3"=>$priceRow." ".$this->sSYSTEM->sCONFIG['sCURRENCY'],
			"4"=>$basketRow["quantity"],
			"5"=>$amountRow." ".$this->sSYSTEM->sCONFIG['sCURRENCY']));
...
class text_table
{
	...
	// Inserts new column
	public function add_col ($colname,$length,$mode)
	{
		$this->cols[0][] = array("name"=>$colname,"length"=>$length,"align"=>$mode);
	}

	// Inserts new row
	public function add_row ($rowdata){		

		$this->rows++;
		$this->cols[$this->rows] = $rowdata;

	}

	public function render_table ()
	{ 
		foreach ($this->cols as $row){	// takes a row
			foreach ($row as $column){	// takes a column
				// Header
				if (!$i){
					if (!$a) $a="0";
					$length[0][$a] = $column["length"];
					$mode[0][$a] = $column["align"];			

					unset ($spaces);
					if ($length[0][$a] > strlen($column["name"])){
						for ($b=0;$b<($length[0][$a]-strlen($column["name"]));$b++) { $spaces .= " "; }
					}
					$length[0][$a] = strlen($column["name"].$spaces);
					$header .= " | ".$column["name"].$spaces;
				}

				// Data
				else {
					unset ($spaces);
					if ($length[0][$a] < strlen($column)) { $column = substr($column,0,$length[0][$a]-4)."[..]";}
					else {
					for ($b=0;$b<($length[0][$a]-strlen($column));$b++){ $spaces.= " "; }}
					if ($mode[0][$a]=="FORMAT_RIGHT"){
						$data .= " | ".$spaces.$column;
					}else {
						$data .= " | ".$column.$spaces;
					}
			...
			$length_of_a_row = strlen($header);
			unset($seperator);
			
			for ($c=0;$c<$length_of_a_row;$c++) $seperator .= "-";
