...
$sql = "SELECT a.categoryID AS category,a.id as articleID,a.active AS active, ordernumber,datum,additionaltext, shippingfree, description_long, aSupplier.name AS supplierName, aSupplier.id AS supplierID,aSupplier.img AS supplierImg, a.name AS articleName, price, sales, pseudoprice, tax,
...
$countArticles = @mysql_num_rows($getAllArticles);

$nodes["totalCount"] = $countArticles;
if (@mysql_num_rows($getArticles)){
	while ($article=mysql_fetch_array($getArticles)){
		...
		$nodes["articles"][] = array('property'=>htmlentities($article["articleName"]), 'value'=>$article["articleID"],"ordernumber"=>htmlentities($article["ordernumber"]),"supplier"=>htmlentities($article["supplierID"]));
