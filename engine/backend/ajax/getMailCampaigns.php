...
switch ($requestType){
	...
	case "CAMPAIGN":
		$getElements = mysql_query("
		SELECT id, type AS description, description AS fallback FROM s_campaigns_containers WHERE promotionID=$requestElement ORDER BY position ASC
		");
		break;
	case "ARTICLES":
		$getElements = mysql_query("
		SELECT id, name AS description FROM s_campaigns_articles WHERE parentID=$requestElement ORDER BY position ASC
		");
		break;
	case "LINKS":
		$getElements = mysql_query("
		SELECT id, description FROM s_campaigns_links WHERE parentID=$requestElement ORDER BY position ASC
		");
		break;
}
	...
	while ($element=mysql_fetch_array($getElements)){
		...
		switch ($requestType){
			case "CAMPAIGNS":
				...
				SELECT id FROM s_campaigns_containers WHERE promotionID={$element["id"]} LIMIT 1
				");
				$type = "CAMPAIGN";
			...
			case "CAMPAIGN":
					...
					if ($element["description"]=="ctBanner"){
						...
						$getCategoryLeafs = mysql_query("
						SELECT id FROM s_campaigns_links WHERE parentID={$element["id"]} LIMIT 1
						");
						...
						$getCategoryLeafs = mysql_query("
						SELECT id FROM s_campaigns_articles WHERE parentID={$element["id"]} LIMIT 1
						");
						$type = "ARTICLES";
					}elseif ($element["description"]=="ctText"){
						$element["description"] = "Text";
						$type = "TEXT";
					}elseif ($element["description"]=="ctVoucher"){
						$element["description"] = "Gutschein";
						$type = "VOUCHER";
					}elseif ($element["description"]=="ctSuggest"){
		...
		if (@mysql_num_rows($getCategoryLeafs)){
			...
			$nodes[] = array('text'=>$element["description"], id=>$type.":".$element["id"], dbId=>$element["id"],parentId=>$element["parent"], type=>$type, leaf=>true, cls=>'folder');
		}
	}
}
