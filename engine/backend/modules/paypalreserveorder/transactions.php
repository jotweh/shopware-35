...
// *****************
//include("../../../connectors/united_transfer/includes/class.uos.php");
...
<title><?php echo $sLang["paypalreserveorder"]["transactions_reorder"] ?></title>
...
if ($_POST["filterReserved"]){
  ...
  if ($booked=='0') {
  	$addReserveFilter = "
  	AND paypal_orders.paymentStatus = 'Pending'
  	";
  }  elseif ($booked=='1') {
  	$addReserveFilter = "
  	AND paypal_orders.paymentStatus = 'Completed'
  	";
  }
}
...
if ($_POST["sTransaction"]){
  ...
  FROM s_order, paypal_orders, s_core_states
	WHERE (
  ...
  $addReserveFilter
	...
	AND s_order.transactionID = paypal_orders.stransId
...
var headers = [
{
"text":"<?php echo $sLang["paypalreserveorder"]["transactions_status"] ?>",
"key":"kdnr","sortable":true,
"fixedWidth":true,"defaultWidth":"500px"}
];
var data = [{"kdnr":"<?php echo $sLang["paypalreserveorder"]["transactions_no_orders_found"] ?>"}
];
...
var headers = [
{
"text":"<?php echo $sLang["paypalreserveorder"]["transactions_date"] ?>",
"key":"regdate","sortable":true,
"fixedWidth":false,"defaultWidth":"115px","date2":true},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_ordernumber"] ?>",
"key":"ordernumber","sortable":true,
"fixedWidth":false,"defaultWidth":"90px"},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_Action"] ?>",
"key":"transaction","sortable":true,
"fixedWidth":false,"defaultWidth":"90px"},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_order_status"] ?>",
"key":"state","sortable":true,
"fixedWidth":false,"defaultWidth":"90px"},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_payment_status"] ?>",
"key":"state2","sortable":true,
"fixedWidth":false,"defaultWidth":"90px"},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_total"] ?>",
"key":"amount","sortable":true,
"fixedWidth":false,"defaultWidth":"90px", "numeric":true},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_customer"] ?>",
"key":"customer","sortable":true,
"fixedWidth":false,"defaultWidth":"150px"},
{"text":"<?php echo $sLang["paypalreserveorder"]["transactions_options"] ?>",
...
<?php
	...
	while ($order=mysql_fetch_array($queryOrders)){
		...
		if ($daysLeft>0 && $order["paymentStatus"]=='Pending'){
    		  $cancel = '<a href=\"\" onclick=\"parent.parent.loadSkeleton(\'paypalreserveorder_action\', false, {\'ordernr\':'.$order['ordernumber'].'}); return false;\" style=\"cursor:pointer\">'.$sLang["paypalreserveorder"]["transactions_PP_free"].'</a> ('.$daysLeft.' Tage)';
		}elseif ($order["paymentStatus"]=='Completed' || $order["paymentStatus"]=='Refunded') {
		      $cancel = '<a href=\"\" onclick=\"parent.parent.loadSkeleton(\'paypalrefund\', false, {\'ordernr\':'.$order['ordernumber'].'}); return false;\" style=\"cursor:pointer\">'.$sLang["paypalreserveorder"]["transactions_PP_refund"].'</a>';
	    } elseif ($daysLeft <= 0 && $order["paymentStatus"]=='Pending') {
			$cancel = $sLang["paypalreserveorder"]["transactions_Period_end"];
		}else {
			$cancel = '';
		}
?>
...
<tr>
<td><?php echo $sLang["paypalreserveorder"]["transactions_Evaluation_of"] ?></td>
...
<td><?php echo $sLang["paypalreserveorder"]["transactions_Evaluation_until"] ?></td>
...
<tr>
<td><?php echo $sLang["paypalreserveorder"]["transactions_Booking_Status"] ?></td>
<td>
<select name="filterReserved">
<option value="0"><?php echo $sLang["paypalreserveorder"]["transactions_show_all"] ?></option>
<option value="1" <?php if ($_POST['filterReserved']==1) echo 'selected'?>><?php echo $sLang["paypalreserveorder"]["transactions_open_bookings"] ?></option>
<option value="2" <?php if ($_POST['filterReserved']==2) echo 'selected'?>><?php echo $sLang["paypalreserveorder"]["transactions_Completed_bookings"] ?></option>
</select>
...
<tr>
<td><?php echo $sLang["paypalreserveorder"]["transactions_status_1"] ?></td>
<td>
...
<td><?php echo $sLang["paypalreserveorder"]["transactions_status_payment"] ?></td>
<td>
...
<tr>
<td><?php echo $sLang["paypalreserveorder"]["transactions_search"] ?></td>
...
<td colspan="4">
 	<div class="buttons" id="buttons" style="width:150px">
		<ul>
		  <li style="display: block;" class="buttonTemplate" id="add"><button onclick="$('reloadSearch').submit();" class="button" id="addArticle" name="" type="button" value="" class="button"><div class="buttonLabel"><?php echo $sLang["paypalreserveorder"]["transactions_refresh_view"] ?></div></button></li>
		</ul>
	</div>
</td>
</tr>
...
<strong><?php echo $sLang["paypalreserveorder"]["transactions_attention"] ?></strong>
<br /><br />
<div id='test'></div>
<?php echo $sLang["paypalreserveorder"]["transactions_total_in_period"] ?> <?php echo $sCore->sFormatPrice(round($invoiceTotal,2)) ?> €<br />
<?php echo $sLang["paypalreserveorder"]["transactions_count_of_orders"] ?> <?php echo$countOrdersTemp; ?>
...
<script>
				...
				function refreshStateChange (ev){
					...
					if (!newStateDescription){
						alert ("<?php echo $sLang["paypalreserveorder"]["transactions_cant_load_Description"] ?>");
						return false;
					}
					...
					if (!orderID){
						alert ("<?php echo $sLang["paypalreserveorder"]["transactions_cant_load_orderID"] ?>");
						return false;
					}
					
					// Do AJAX - Call
					new Ajax('http://<?php echo $sCore->sCONFIG['sBASEPATH']?>/engine/backend/ajax/changeOrderState.php?artID='+orderID+'&value='+newState+'&field=status', {method: 'get',onComplete: function(el){
					switch (el){
						case "ERROR":
							parent.Growl("<?php echo $sLang["paypalreserveorder"]["transactions_cant_refresh_status"] ?>");
						break;
						default:
							parent.Growl("<?php echo $sLang["paypalreserveorder"]["transactions_status_order"] ?> "+orderNumber+" <?php echo $sLang["paypalreserveorder"]["transactions_has_left"] ?> "+newStateDescription+" <?php echo $sLang["paypalreserveorder"]["transactions_changed"] ?>");
					}
				...
				function refreshPaymentChange (ev){
					...
					// Do AJAX - Call
					new Ajax('http://<?php echo $sCore->sCONFIG['sBASEPATH']?>/engine/backend/ajax/changeOrderState.php?artID='+orderID+'&value='+newState+'&field=cleared', {method: 'get',onComplete: function(el){
					switch (el){
						case "ERROR":
							parent.Growl("<?php echo $sLang["paypalreserveorder"]["transactions_cant_refresh_status"] ?>");
						break;
