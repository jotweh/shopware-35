		...
		case "deleteArticle":
			...
			//ndow.location.href = "<?php echo $_SERVER["PHP_SELF"] ?>?group=<?php echo $_GET["group"]?>&delete="+sId;
				...
				Ext.Ajax.request({
	                ...
	                url: '<?php echo $_SERVER["SERVER_PORT"] == "80" ? "http" : "https" ?>://<?php echo $sCore->sCONFIG['sBASEPATH']?>/engine/backend/ajax/deleteSite.php', 
	                params: { 
	                    nodeId: sId								
	                },
	                failure:function(response,options){
	                    Ext.MessageBox.alert('Seite konnte nicht gelöscht werden');
	                },                             
	                success:function(response,options){
						parent.Growl('Seite wurde gelöscht');
						myExt.reload();
						
						//Clear center
						var url = "cms.php?hide";
...
function deleteArticle(ev,text){
	...
	init : function(){
		...
		rootVisible:false,
		lines:false,
		loader: new Ext.tree.TreeLoader({dataUrl:'../../../backend/ajax/getCmsGroups.php'}),
		autoScroll:true,
	    ...
	    tree.on('click', function(e){
 			...
 			{
 				var url = "cms.php?edit="+e.attributes.id;
				Ext.get('framepanelmyiframe').dom.src = url;
