...
$sLang["clickpay"]["transactions_reorder"] = "Reorder TreePanel";
$sLang["clickpay"]["transactions_status"] = "Status";
$sLang["clickpay"]["transactions_no_orders_found"] = "Keine Bestellungen gefunden";
$sLang["clickpay"]["transactions_date"] = "Datum";
$sLang["clickpay"]["transactions_ordernumber"] = "Bestellnr.";
$sLang["clickpay"]["transactions_Action"] = "Transaktion.";
$sLang["clickpay"]["transactions_order_status"] = "Bestellstatus";
$sLang["clickpay"]["transactions_payment_status"] = "Zahlstatus";
$sLang["clickpay"]["transactions_total"] = "Gesamtbetrag";
$sLang["clickpay"]["transactions_customer"] = "Kunde";
$sLang["clickpay"]["transactions_options"] = "Optionen";
$sLang["clickpay"]["transactions_PP_free"] = "Buchung";
$sLang["clickpay"]["transactions_Period_end"] = "Frist abgelaufen";
$sLang["clickpay"]["transactions_Evaluation_of"] = "Auswertung von:";
$sLang["clickpay"]["transactions_Evaluation_until"] = "Auswertung bis:";
$sLang["clickpay"]["transactions_Booking_Status"] = "Buchungsstatus:";
$sLang["clickpay"]["transactions_show_all"] = "Alle anzeigen";
$sLang["clickpay"]["transactions_open_bookings"] = "Offene Buchungen";
$sLang["clickpay"]["transactions_Completed_bookings"] = "Abgeschlossene Buchungen";
$sLang["clickpay"]["transactions_status_1"] = "Status:";
$sLang["clickpay"]["transactions_status_payment"] = "Bezahlstatus:";
$sLang["clickpay"]["transactions_search"] = "Suche (Nr./Transaktion)";
$sLang["clickpay"]["transactions_refresh_view"] = "Ansicht aktualisieren";
$sLang["clickpay"]["transactions_attention"] = "Hinweis: Durch Doppelklick auf den Status lässt sich dieser ändern. <br/ > Shopwware empfiehlt grundsätzlich reservierte Zahlungen nach spätestens 3 Tagen freizugeben (belasten).";
$sLang["clickpay"]["transactions_total_in_period"] = "Gesamtumsatz in Zeitraum:";
$sLang["clickpay"]["transactions_count_of_orders"] = "Anzahl Bestellungen:";
$sLang["clickpay"]["transactions_cant_load_Description"] = "Beschreibung konnte nicht geladen werden";
$sLang["clickpay"]["transactions_cant_load_orderID"] = "Bestell-ID konnte nicht ermittelt werden";
$sLang["clickpay"]["transactions_cant_refresh_status"] = "Status konnte nicht aktualisiert werden";
$sLang["clickpay"]["transactions_status_order"] = "Der Status der Bestellung";
$sLang["clickpay"]["transactions_has_left"] = "wurde auf";
$sLang["clickpay"]["transactions_changed"] = "geändert!";

/*
mysql_close();
session_write_close();

require_once(dirname(dirname(dirname(__FILE__))).'/connectors/clickpay/clickpay.class.php');
...
<title><?php echo $sLang["clickpay"]["transactions_reorder"] ?></title>
...
if (!empty($_POST["filterReserved"])){
	$addReserveFilter = "
  		AND ro.status = ".($_POST["filterReserved"]-1)."
  	";
}
...
if ($_POST["sTransaction"]){
	...
	SELECT  s_order.id AS id,
          ...
          ro.status as clickpay_status,
          p.description as paymentmean
  FROM s_order, eos_reserved_orders ro, s_core_states, s_core_paymentmeans p
	WHERE (
	...
	AND s_core_states.id = s_order.status
 	AND p.id=s_order.paymentID
 	AND s_order.status != -1
	$addPaymentFilter
	$addStatusFilter
  $addReserveFilter
	AND s_order.transactionID != ''
	AND s_order.transactionID = ro.transactionID
...
var headers = [
{
"text":"<?php echo $sLang["clickpay"]["transactions_status"] ?>",
"key":"kdnr","sortable":true,
"fixedWidth":true,"defaultWidth":"500px"}
];
var data = [{"kdnr":"<?php echo $sLang["clickpay"]["transactions_no_orders_found"] ?>"}
];
...
var headers = [
{
"text":"<?php echo $sLang["clickpay"]["transactions_date"] ?>",
"key":"regdate","sortable":true,
"fixedWidth":false,"defaultWidth":"115px","date2":true},
{"text":"<?php echo $sLang["clickpay"]["transactions_ordernumber"] ?>",
"key":"ordernumber","sortable":true,
"fixedWidth":false,"defaultWidth":"80px"},
{"text":"<?php echo $sLang["clickpay"]["transactions_Action"] ?>",
"key":"transaction","sortable":true,
"fixedWidth":false,"defaultWidth":"80px"},
{"text":"<?php echo $sLang["clickpay"]["transactions_order_status"] ?>",
"key":"state","sortable":true,
"fixedWidth":false,"defaultWidth":"80px"},
{"text":"<?php echo $sLang["clickpay"]["transactions_payment_status"] ?>",
"key":"state2","sortable":true,
"fixedWidth":false,"defaultWidth":"90px"},
{"text":"ClickPay Status",
"key":"clickpay_status","sortable":true,
"fixedWidth":false,"defaultWidth":"90px"},
{"text":"Zahlungsart",
"key":"paymentmean","sortable":true,
"fixedWidth":false,"defaultWidth":"120px"},
{"text":"<?php echo $sLang["clickpay"]["transactions_total"] ?>",
"key":"amount","sortable":true,
"fixedWidth":false,"defaultWidth":"80px", "numeric":true},
{"text":"<?php echo $sLang["clickpay"]["transactions_customer"] ?>",
"key":"customer","sortable":true,
"fixedWidth":false,"defaultWidth":"150px"},
{"text":"<?php echo $sLang["clickpay"]["transactions_options"] ?>",
...
<?php
	...
	while ($order=mysql_fetch_array($queryOrders)){
		...
		if ($daysLeft>0 && $order["saferpay_complete"]=='0'){
    		  $cancel = '<a href=\"\" onclick=\"parent.parent.loadSkeleton(\'clickpay_action\', false, {\'ordernr\':'.$order['ordernumber'].'}); return false;\" style=\"cursor:pointer\">'.$sLang["clickpay"]["transactions_PP_free"].'</a> ('.$daysLeft.' Tage)';
		} elseif ($daysLeft <= 0 && $order["saferpay_complete"]=='0') {
			$cancel = $sLang["clickpay"]["transactions_Period_end"];
		}else {
			$cancel = '';
		}
		*/
		$cancel = '<a href=\"\" class=\"ico pencil\" onclick=\"parent.parent.loadSkeleton(\'clickpay_detail\', false, {\'orderID\':'.$order['id'].'}); return false;\" style=\"cursor:pointer\"></a>';
		
		if(!empty($sClickPayStatus[$order["clickpay_status"]]))
			$clickpay_status = $sClickPayStatus[$order["clickpay_status"]];
		else 
			$clickpay_status = "";
?>

	{"orderID":"<?php echo $order["id"]?>","paymentmean":"<?php echo $order["paymentmean"]?>","transaction":"<?php echo $order["transactionID"] ?>","stateNumeric":"<?php echo $order["status"] ?>","paymentNumeric":"<?php echo $order["cleared"] ?>","regdate":"<?php echo $order["ordertimeFormated"]?>","ordernumber":"<a onclick=\"parent.parent.loadSkeleton('orders',false, {'id':<?php echo $order["id"] ?>})\" class=\"ico information\" style=\"cursor:pointer\"></a><?php echo $order["ordernumber"]?>","state":"<?php echo $status ?>","state2":"<?php echo $payment ?>","clickpay_status":"<?php echo $clickpay_status ?>","amount":"<?php echo number_format($order["invoice_amount"], 2, '.', '')?> ","customer":"<a class=\"ico user\" style=\"cursor:pointer\" onclick=\"parent.parent.loadSkeleton('userdetails',false, {'user':<?php echo $order["userID"] ?>})\"></a><?php echo $customer ?>","order3":"<?php echo $cancel?>"}<?php echo $comma ?>
<?php
	} // for each user
	echo "];";
...
<tr>
<td><?php echo $sLang["clickpay"]["transactions_Evaluation_of"] ?></td>
...
<td><?php echo $sLang["clickpay"]["transactions_Evaluation_until"] ?></td>
...
<tr>
<td>ClickPay Status</td>
<td>
<select name="filterReserved">
<option value="0"><?php echo $sLang["clickpay"]["transactions_show_all"] ?></option>
<?php foreach ($sClickPayStatus as $statusID=>$status):?>
<option value="<?php echo $statusID+1;?>"<?php if ($_POST['filterReserved']==$statusID+1) echo ' selected'?>><?php echo $status?></option>
<?php endforeach;?>
</select>
...
<tr>
<td><?php echo $sLang["clickpay"]["transactions_status_1"] ?></td>
<td>
...
<tr>
<td><?php echo $sLang["clickpay"]["transactions_search"] ?></td>
...
<td colspan="4">
 	<div class="buttons" id="buttons" style="width:150px">
		<ul>
		  <li style="display: block;" class="buttonTemplate" id="add"><button onclick="$('reloadSearch').submit();" class="button" id="addArticle" name="" type="button" value="" class="button"><div class="buttonLabel"><?php echo $sLang["clickpay"]["transactions_refresh_view"] ?></div></button></li>
		</ul>
	</div>
</td>
</tr>
...
<strong><?php echo $sLang["clickpay"]["transactions_attention"] ?></strong>
<br /><br />
<div id='test'></div>
<?php echo $sLang["clickpay"]["transactions_total_in_period"] ?> <?php echo $sCore->sFormatPrice(round($invoiceTotal,2)) ?> €<br />
<?php echo $sLang["clickpay"]["transactions_count_of_orders"] ?> <?php echo$countOrdersTemp; ?>
...
<script>
				...
				function refreshStateChange (ev){
					...
					if (!newStateDescription){
						alert ("<?php echo $sLang["clickpay"]["transactions_cant_load_Description"] ?>");
						return false;
					}
					...
					if (!orderID){
						alert ("<?php echo $sLang["clickpay"]["transactions_cant_load_orderID"] ?>");
						return false;
					}
					...
					switch (el){
						case "ERROR":
							parent.Growl("<?php echo $sLang["clickpay"]["transactions_cant_refresh_status"] ?>");
						break;
						default:
							parent.Growl("<?php echo $sLang["clickpay"]["transactions_status_order"] ?> "+orderNumber+" <?php echo $sLang["clickpay"]["transactions_has_left"] ?> "+newStateDescription+" <?php echo $sLang["clickpay"]["transactions_changed"] ?>");
					}
