...
<head>
  <title><?php echo $sLang["user"]["user_user_list"] ?></title>
			...
			// Check if document already exists
				Ext.Ajax.request({
					...
					success: function ( result, request ) {
						...
						Ext.getCmp('install_progress').updateProgress(0.1, "Beleg-Erstellung läuft...");
	...
	deleteOrderDetail : function(id, orderID, name){
		Ext.MessageBox.confirm('Bestätigung', 'Soll die Position '+name+' wirklich gelöscht werden?', function(btn){
			if(btn == "yes")
			{
				Ext.Ajax.request({
				   ...
				   success: function(){
						...
						parent.Growl('Löschvorgang erfolgreich!');
				   }
				});
			}else{
				parent.Growl('Löschvorgang abgebrochen!');
			}
		});
	},
		...
		Ext.MessageBox.confirm('<?php echo $sLang["orderlist"]["del_order_confirmtitle"]; ?>', '<?php echo $sLang["orderlist"]["del_order_confirmtxt_1"]; ?> '+ordernumber+' <?php echo $sLang["orderlist"]["del_order_confirmtxt_2"]; ?> '+customer+' <?php echo $sLang["orderlist"]["del_order_confirmtxt_3"]; ?>', function(btn){
			if(btn == 'yes')
			{
				Ext.Ajax.request({
				   ...
				   success: function(){
				   		parent.Growl('<?php echo $sLang["orderlist"]["del_order_acknowltxt_1"]; ?> '+ordernumber+' <?php echo $sLang["orderlist"]["del_order_acknowltxt_2"]; ?> '+customer+' <?php echo $sLang["orderlist"]["del_order_acknowltxt_3"]; ?>');
				   		Ext.getCmp('orderlist_grid').store.load();
				   },
				   params: { 'delete': id }
				});
			}else{
				parent.Growl('<?php echo $sLang["orderlist"]["del_order_cancel"]; ?>');
			}
		});
	},
	init : function(){
       ...
       store = new Ext.data.Store({
	        url: '<?php echo $_SERVER["SERVER_PORT"] == "80" ? "http" : "https" ?>://<?php echo $sCore->sCONFIG['sBASEPATH']?>/engine/backend/ajax/getOrders.php',
	...
	*/
    ...
    var windowPanel  = 
		...
		*/
		...
		var windowForm = new Ext.FormPanel({
	      ...
	      new Ext.form.ComboBox({
			    ...
			    id: 'docChangeState',
			    triggerAction: 'all',
			    emptyText:'Nicht ändern',
			    selectOnFocus:true,
			    value:-1
	      }),    
	      new Ext.form.ComboBox({
	      		fieldLabel: 'Zahlstatus ändern auf',
			    store: paystoreForm,
			    displayField:'state',
			    valueField:'id',
			    typeAhead: true,
	   ...
	   myExt.window = new Ext.Window({
	        ...
	        {
	            text: 'Schließen', handler: function (){
    ...
    function renderOptions(value, p, r){
		...
		'<a class="ico pencil" style="cursor:pointer" onclick="myExt.openClient({0},{1})"></a><a class="ico pencil_arrow" style="cursor:pointer" onclick="parent.loadSkeleton({2},false,{3})"></a><a class="ico delete" style="cursor:pointer" onclick="myExt.deleteOrder({4},{5},{6})"></a>',id,"'"+name+"'","'orders'","{'id':"+r.data.id+"}",id,"'"+ordernumber+"'","'"+customer+"'"
		);
    }
    ...
    var grid = new Ext.grid.EditorGridPanel({
        ...
        trackMouseOver:true,
        //Ext Bug Using Hotfix "empetyFn"
        sm: new Ext.grid.RowSelectionModel({selectRow:Ext.empetyFn}),
        loadMask: true,
        stripeRows: true,
   ...
   function limitFilter () {
	...
	var checkboxgroup3 = {
		...
		items: [<?php
		$result = mysql_query("
			SELECT id, description FROM s_core_paymentmeans ORDER BY position ASC
		");
		$order_status3 = array();
		if($result&&mysql_num_rows($result))
		while ($status = mysql_fetch_assoc($result)){
		...
		$a = 0;
		foreach ($order_status3 as $id=>$value){
			if($a != 0 && $a < count($order_status3)) echo ",";
			?>{boxLabel: '<?php echo$value?>', name: '<?php echo$id?>', checked: false}<?php
			$a++;
		}
		?>]
...
<form enctype="multipart/form-data" method="post" id="form" name="form" action="#">	
	...
	<a href="#" class="ico cross" onclick="StageChangeMailClose();" style="float:right; cursor: pointer;"></a>
</form>
</div>
	...
	new Ajax('<?php echo $_SERVER["SERVER_PORT"] == "80" ? "http" : "https" ?>://<?php echo $sCore->sCONFIG['sBASEPATH']?>/backend/OrderState/read?id='+orderID+'&status='+newState, {method: 'get',onComplete: function(mail){
		...
		if (newState==7 && payment == "hanseatic.tpl"){
			new Ajax('<?php echo $_SERVER["SERVER_PORT"] == "80" ? "http" : "https" ?>://<?php echo $sCore->sCONFIG['sBASEPATH']?>/engine/connectors/hanseatic/deliver.php?oid='+ordernumber, {method: 'get',onComplete: function(mail){
				parent.Growl("Bestellstatus wurde an Hanseatic übermittelt");
			}}).request();
		}
	}}).request();
}
