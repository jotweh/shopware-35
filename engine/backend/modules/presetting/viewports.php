...
$valueName = $sLang["presettings"]["viewport_Viewport"];
$valueDelete = true;
$valueTable = "s_core_viewports";
$valueAdd = true;
$valueDescription = "description";
//$valueWhere = "WHERE id>=1";
$substitute = $sLang["presettings"]["viewport_array"];
...
if ($_GET["delete"]){
	...
	$sInform = "$valueName ".$sLang["presettings"]["viewport_was_deleted"];
}
...
if ($_POST["sAction"]=="saveArticle"){
	...
	if (!$sError){
		...
		*/
		...
		if ($insertArticle){
			$sInform = $sLang["presettings"]["viewport_entry_saved"];
		}else {
...
if ($_GET["edit"]){
	...
	if (!@mysql_num_rows($getSite)){
		$sInform = "$valueName ".$sLang["presettings"]["viewport_cant_be_found"];
	}else {		
...
<body >
<?php
$sCore->sInitTranslations(1,"config_viewports","true");
?>
<script>
...
function deleteCustomerGroup(ev,text){
		parent.parent.sConfirmationObj.show('<?php echo $sLang["presettings"]["viewport_should"] ?> <?php echo $valueName ?> "'+text+'" <?php echo $sLang["presettings"]["viewport_really_deleted"] ?>',window,'deleteCustomer',ev);
	}
...
<form enctype="multipart/form-data" method="POST" id="ourForm" action="<?php echo $_SERVER["PHP_SELF"]."?id=".$_GET["id"]?>&edit=<?php echo $_GET["edit"]?>">
		...
		<fieldset>
		<legend><?php echo $_GET["new"] ? "$valueName ".$sLang["presettings"]["viewport_create"] : "$valueName ".$sLang["presettings"]["viewport_edit"] ?></legend>
		<ul>
		...
		while ($row = mysql_fetch_assoc($getFields)) {
		   	...
		   	$fieldName = $row["Field"];
		   	   ...
		   	   if ($fieldName=="description"){
			   	   ...
			   	   if ($_GET["edit"]){
			   	   	   echo $sCore->sBuildTranslation("$fieldName","$fieldName","1","config_viewports","{$getCustomerGroup["viewport"]}");
				   }
				echo "</li>";
		   	   }else {
	...
	<div class="buttons" id="buttons">
		<ul>
		<li id="buttonTemplate" class="buttonTemplate"><button onClick="$('ourForm').submit();" type="submit" value="send" class="button"><div class="buttonLabel"><?php echo $sLang["presettings"]["viewport_save"] ?></div></button></li>	
...
<legend><?php echo $sLang["presettings"]["viewport_Available_records"] ?></legend>
<p><img src="../../../backend/img/default/icons/information.png" style="margin:0 15px 0 0;" /><?php echo $sLang["presettings"]["viewport_Please_change_this_data_only"] ?></p>
<table cellpadding="0" cellspacing="0" class="listing">
   <tbody>
   		 <?php	
			// Query Related-Articles
			$sql = "
			SELECT id, $valueDescription as description, viewport FROM $valueTable $valueWhere ORDER BY description ASC
			";
     ...
     <tr class="rowcolor2">
       <th class="first-child"><img src="../../../backend/img/default/icons4/socket.png" style="margin:0 15px 0 0;" /><?php echo $article["description"]?> (<?php echo $article["viewport"]?>)</th>
...
?>
	<div class="buttons" id="buttons" style="margin-left:5px;">
		...
		<li id="buttonTemplate" class="buttonTemplate"><a href="<?php echo $PHP_SELF."?new=1" ?>" class="bt_icon socket" style="text-decoration:none;"><?php echo $valueName ?> <?php echo $sLang["presettings"]["viewport_create"] ?></a></li>	
